...
**📓 每次对话都有完整复盘**
做了什么、用了什么思路、✅ 做得好的地方、⚠️ 下次可以改进的地方、涉及到的新概念、对话中出现的所有项目链接——一次对话，一份复盘，什么都不漏。

**🃏 专属错题本**
踩过的坑、走过的弯路、Claude 理解错误的地方，全部分类归档。下次遇到同类问题，翻一下，不用再摔同一个跟头。

**📚 概念库越用越厚**
每次对话涉及的工具和概念，自动追加到同一个页面里。MCP、launchd、GitHub Actions……用得越多，页面越厚，知识越深。

**🙋 问过的所有问题，系统整理**
每次问过 Claude Code 的操作问题，自动归入问题手册，按类别整理（Claude Code 操作 / Git & GitHub / 通用概念）。再也不用在几十个对话窗口里翻"我当时怎么解决的"。

**📅 每周使用周报**
每周自动汇总：做了哪些项目、学了哪些新东西、犯了哪些错——让你清楚看到自己的成长轨迹。
